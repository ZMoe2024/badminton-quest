...
在当前 Railway 实例上测试了独立临时浏览器，未收集用户登录凭据、未预约或支付：

- 容器拒绝 `unshare -Ur`，不能直接沿用桌面 Chrome 所需的用户命名空间隔离。
- Firefox 有界面模式能启动，但学校预约首页返回 HTTP 412 后转为 HTTP 400，正文为空，未到统一认证界面。
- 用普通 Firefox 启动方式再作对照，不连接自动化或远程调试，16 秒后的画面仍为空白。

这证明当前部署上的云端登录尚不可用，不能据此断言唯一原因是服务器 IP 或自动化检测。暂不对手机用户开放云端登录入口；电脑本地提取与手动导入保持可用。临时浏览器依赖未加入 Dockerfile，通过重新部署清理测试容器。后续需要在支持浏览器隔离、且能正常打开学校首页的环境重新验收。
